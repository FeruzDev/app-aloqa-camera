...
const Home = () => {
    ...
    const getStatistik1 = (from_date, to_date) => {
        axios.get(API_PATH + "company/" + localStorage.getItem('id') + "/hr/attendance/statistics/?from_date=" + from_date + "&to_date=" + to_date + "&building_id=" + building_id + "&department_id=" + department_id + Number(time?.getDate()), CONFIG)
            .then(res => {
                // setData(res?.data)
    ...
    useEffect(() => {
        ...
        getBuilding()
        // getStatistik()
        getStatistik1(Number(time?.getFullYear()) + "-" + Number(time?.getMonth() + 1) + "-" + Number(time?.getDate()), time?.getFullYear() + "-" + Number(time?.getMonth() + 1) + "-" + Number(tomorrow.toDateString().slice(8, 10)))
                    ...
                    <div className="radio-list">
                        <div className="left-items">
                            <FormControl>
                                ...
                                >
                                    <FormControlLabel value="female" control={<Radio onChange={() => settopType(2)}/>}
                                                      label="Приходов"/>
                                    <FormControlLabel value="male" control={<Radio onChange={() => settopType(3)}/>}
                                                      label="Уходов"/>
                                    <FormControlLabel value="other" control={<Radio onChange={() => settopType(1)}/>}
                                                      label="Все"/>
                                </RadioGroup>
                            </FormControl>
                        </div>
                            ...
                            <FormControl>
                                ...
                                >
                                    <FormControlLabel value="female" control={<Radio
                                        onChange={() => getStatistik1(time?.getFullYear() + "-" + Number(time?.getMonth() + 1) + "-" + Number(time?.getDate()), time?.getFullYear() + "-" + Number(time?.getMonth() + 1) + "-" + tomorrow.toDateString().slice(8, 10))}/>}
                                                      label="Сегодня"/>
                                    <FormControlLabel value="male" control={<Radio
                                        onChange={() => getStatistik1(time?.getFullYear() + "-" + Number(time?.getMonth() + 1) + "-" + Number(time?.getDate()), "2023-11-19")}/>}
                                                      label="На этой неделе"/>
                                    <FormControlLabel value="other" control={<Radio
                                        onChange={() => getStatistik1(time?.getFullYear() + "-" + Number(time?.getMonth() + 1) + "-" + Number(time?.getDate()), "2023-11-30")}/>}
                                                      label="Этот месяц"/>
                                </RadioGroup>
                            </FormControl>
                        </div>
